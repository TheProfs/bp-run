...
const format = (template, args = []) => args.length === 0
    ? template
...
const log = {
    error: (msg, args = []) => process.env.NODE_ENV !== 'test' &&
        console.error(`${color('red', 'error:')} ${format(msg, args)}`),
    success: (msg, args = []) => process.env.NODE_ENV !== 'test' &&
        console.error(`${color('green', '✓')} ${format(msg, args)}`),
    warning: (msg, args = []) => process.env.NODE_ENV !== 'test' &&
        console.error(`${color('yellow', 'warning:')} ${format(msg, args)}`),
    info: (msg, args = []) => process.env.NODE_ENV !== 'test' &&
...
class User {
    ...
    save = async () => {
        const changed = Object.keys(this).filter(key => !key.startsWith('_') &&
            key !== 'customer' &&
...
const getKeychain = async (key, run = execFileDefault) => {
    ...
    try {
        const { stdout } = await run('/usr/bin/security', ['find-generic-password', '-s', 'BP_CLI', '-a', key, '-w']);
        return stdout.trim();
    }
    catch (err) {
...
const ensureKeychain = async (key, run = execFileDefault, promptFn = prompt) => {
...
const stripeGenerator = async function* (resource, method, filters, name) {
...
const createUsersGenerator = (db, stripe) => {
    return async function* (where = '', params = [], { fetchCustomer = false } = {}) {
        const sql = where
        ...
        for (const row of result.rows) {
            ...
            if (fetchCustomer && row.stripe_id) {
                try {
                    ...
                    log.warning('Failed to fetch customer $1: $2', [row.stripe_id, err.message]);
                    user.customer = null;
                }
            }
...
const createQueryHelper = (db) => async (sql, params = []) => {
...
const exec = async (filepath) => {
    ...
    try {
        ...
        await script.runInContext(vm.createContext(context), { microtaskMode: 'afterEvaluate' });
    }
    catch (err) {
...
if (import.meta.url.startsWith('file://') && process.argv[1]) {
    ...
    const isRunningAsScript = import.meta.url === `file://${scriptPath}` ||
...
export { User, stripeGenerator, getKeychain, setKeychain, ensureKeychain, createUsersGenerator, createQueryHelper };
